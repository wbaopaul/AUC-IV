...
var_auc_ig <- function(pi_hat, phi, yr, R, cova1, xid  = 1, auc1){ 
  ...
  for(i in 1:n){
   ...
   s2[i, ] = (pi1 - R[i]) * ax 
   FIM2 = FIM2 - pi1 * (1 - pi1) * outer(ax, ax)
...
est_auc_iv <- function(init_phi, yr, R, cova1, cova2, xid = 1, em_run){  
    ...
    return(list(auc_iv, var_auc, mu, phi))
}
...
est_auc_lzh <- function(inits, yr, R, cova1, cova2, xid = 1){
 ...
 # inits[1] = -1
  ...
  return(list(auc_fp, var_auc, mu, phi))
}
...
est_auc_ig <- function(yr, R, cova1, xid = 1){
  ...
  return(list(auc_ig, var_auc, phi_ig))
}
...
est_auc_f <- function(y, x){
  ...
  auc_f = res$auc
  var_auc = var(res)
  return(list(auc_f, var_auc))
}


## --- the following codes are not used but maybe useful in the future ####
...
## solve the equivalent optimization problem by EM --may not be used
